# Perform Grace test 
# 
# Note: This is code of the Grace test available on Cran on 7/13/16 in Grace package.
# I have changed it to allow option of not-normalizing X columns. 
# Original code contained the following note: 
# 
# "This function calculates Grace coefficients and p-values. 
# Author: Sen Zhao Email: sendavid7@gmail.com" 
# 
# @param Y 
# @param X
# @param lambda.L
...
# @param C
# @param K 
# @return list containing parameter estimates and p-values 
# @import scalreg
# @import glmnet
# 
grace.test.C <- function(Y, X, L, lambda.L, lambda.2 = 0, 
                         normalize.L = FALSE, normalize.X  = TRUE, center.Y = TRUE,
                         eta = 0.05, C = 4 * sqrt(3), K = 10){
  
  requireNamespace("scalreg")
  ...
  sig.L <- scalreg::scalreg(X, Y)$hsigma   # Error standard deviation from the scaled lasso
  lam <- sig.L * C * sqrt(log(p) / n) / 2 # Lasso initial tuning parameter
  beta.init <- glmnet::glmnet(X, Y, lambda = lam, intercept = FALSE)$beta[, 1]   # Initial estimator
  ...
  pval <- 2 * (1 - stats::pnorm(abs(Tstat / se)))
